...
class Panaroma:
    ...
    def image_stitch(self, images, lowe_ratio=0.75, max_Threshold=4.0, match_status=False, rectangled=False):
        ...
        # If the image hasn't been stitched, but instead is just one of the input images
        if (result_image.shape[0] == images[0].shape[0] and result_image.shape[1] == images[0].shape[1]) or (
                result_image.shape[0] == images[1].shape[0] and result_image.shape[1] == images[1].shape[1]):
            raise cv2.error
        timer_end = datetime.datetime.now()
        # Rectangling - post processing
        if not rectangled:
            output_cropped = crop_img(result_image)
        else:
            output_cropped = rectangling(result_image)
        ...
        return output_cropped, (timer_end - timer_start), Homography

    def getwarp_perspective(self, imageA, imageB, Homography):
        # region Modified implementation
        """
        ...
        translate = [-xmin, -ymin]
        result_image = cv2.warpPerspective(imageA, Homography,
                                    (xmax - xmin, ymax - ymin))
        result_image[translate[1]:h2 + translate[1], translate[0]:w2 + translate[0]] = imageB
        """
        # end region
        # original, now redundant
        val = imageA.shape[1] + imageB.shape[1]
        val_y = imageA.shape[0] + imageB.shape[0]
        result_image = cv2.warpPerspective(imageA, Homography, (val, val_y))
