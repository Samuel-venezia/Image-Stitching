...
def transform3(Hmdlt, width_max, width_min, height_max, height_min, grid_w, grid_h):
    ...
    x_map = np.vstack(x_, dtype=np.float16).reshape([out_height, out_width])
    y_map = np.vstack(y_, dtype=np.float16).reshape([out_height, out_width])
...
def imagewarping(img1, img2, H, min_x, max_x, min_y, max_y, C1=None, C2=None):
    ...
    warped_mask1 = np.zeros([out_height, out_width], dtype=np.float16)
